...
class TestSwiftImport(TestCase):
    def setUp(self):
        ...
        seg_id = (
            np.linspace(1, self.n_seg, self._len).astype(int).reshape((self._len, 1))
        )
        track_id = (
            np.linspace(1, self.n_track, self._len).astype(int).reshape((self._len, 1))
        )
    ...
    def test_import_with_sigma(self):
        ...
        [
            self.assertAlmostEqual(np.mean(t.sigma), self.sigma / 1000.0, 1)
            for t in tracks
        ]
