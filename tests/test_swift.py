...
class TestSwiftImport(TestCase):
    ...
    def setUp(self):
        ...
        seg_id = np.linspace(1, self.n_seg, self._len).astype(int).reshape((self._len, 1))
        track_id = np.linspace(1, self.n_track, self._len).astype(int).reshape((self._len, 1))
        
        sigma = np.random.standard_gamma(self.sigma, (self._len, 1))

        self.data = np.concatenate([xy, frame, seg_id, track_id, sigma], axis=1)
        self.df = pd.DataFrame(
            data=self.data, 
            columns=['x [nm]', 'y [nm]', 'frame', 'seg.id', 'track.id', 'sigma']
            )

    def test_data_integrity(self):
        [self.assertEqual(d, v) for d,v in zip(self.data.shape, (self._len, 6))]
    ...
    def test_import_with_sigma(self):
        ...
        [self.assertAlmostEqual(np.mean(t.sigma), self.sigma/1000., 1) for t in tracks]
