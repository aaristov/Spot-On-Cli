...
def concat_all(rep_fov_xyft, min_len=3, exposure_ms=None, pixel_size_um=None):
    ...
    print(
        f"discovered {n_rep} replicates containing {each_len_rep} \
        acquisitions"
    )
    print(
...
def group_tracks(
    ...
    assert (
        xyft.shape[1] == 4
    ), f"wrong number of columns: expected 4, \
    ...
    xyft[:, 3] = xyft[:, 2]  # xyff
    if exposure_ms:
        xyft[:, 2] = xyft[:, 2] * exposure_ms * 1.0e-3  # xytf
...
def analyse_mat_file(
    ...
    **fit_params,
    ...
    def my_fit(rep):
        ...
        fit_result = tools.auto_fit(cell_spt, **fit_params)
        return fit_result
...
def analyse_mat_files(*path_list, exposure_ms=60.0, pixel_size_um=0.075, **kwargs):
    print(path_list)
    stats = {}
    for data_path in path_list[0]:
        stats[data_path] = analyse_mat_file(
            data_path, exposure_ms=exposure_ms, pixel_size_um=pixel_size_um
        )
    return stats


def get_stats(
    reps_fits, save_path=None, print_stats=True, save_fmt="json", suffix=".stats"
):
    # get stats
...
class MatlabTable:
    ...
    def group_fovs(
        self, fovs, min_len=3, exposure_ms=None, pixel_size_um=None, desc=""
    ):
        tracks = []
...
def table_import(path):
    ...
    supported_extensions = {
        ".mat": {
            "description": "matlab table from Gizem. rep.fov.[loc_xyft]",
            "function": read_gizem_mat,
        },
        ".xml": {
            "description": "xml file from Trackmate. ",
