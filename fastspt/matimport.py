...
def concat_all(rep_fov_xyft, min_len=3, exposure_ms=None, pixel_size_um=None):
    ...
    print(f'discovered {n_rep} replicates containing {each_len_rep} \
        acquisitions')
    print(f'Assembling tracks with minimal length {min_len}, \
...
def analyse_mat_files(
    *path_list, exposure_ms=60., pixel_size_um=0.075, **kwargs
):
    print(path_list)
    stats = {}
...
def table_import(path):
    ...
    try:
        ...
        logger.info(
            f"Opening {file_extension} which is expected to be {description}")
    except KeyError:
        logger.error(
