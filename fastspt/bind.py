...
def count_on_off_tracks(tracks, min_len=7, threshold=0.005, on_seq=[1,0,0,0], off_seq=[0,0,0,1]):
    '''
    Counts template occurance in the list of treack with format  [[x1, y1, time1, frame1], [...]].
    Return (kon, koff) for all tracks.
    '''
    if min_len:
        tracks = list(filter(lambda t: len(t) >= min_len, tracks))
    get_counts = lambda track: count_on_off(track, threshold, on_seq, off_seq)
